...
//if ($_GET['mode']=='prefs' && $_GET['action']=='maintenance')
//{
	$action = 'maint';
//}
...
//    var_dump (strtotime('-12 month'));
//		$old_date = intval(mktime(0,0,0,$ecal_class->now_date['mon']-$back_count,1,$ecal_class->now_date['year']));
//    var_dump ($old_date);
		$old_date = intval(strtotime('-'.$back_count.' month'));
...
if (isset($_POST['confirmdeleteold']) && (isset($_GET['backdel'])))
{
//	$old_date = intval($ec_qs[1]);
	$old_date = intval($_GET['backdel']);
//  var_dump ($old_date);
	$old_string = strftime("%d %B %Y",$old_date);
...
if ($action == 'confdel')
{
	...
	$text = "
	<form method='post' action='".e_SELF."?mode=prefs&action=maintenance&backdel={$ec_qs[1]}'>
	<table class='table adminform'>
	<tr>
		<td>".EC_ADLAN_A150.$old_string."</td>
		<td>".$frm->admin_button('confirmdeleteold', LAN_UI_DELETE_LABEL, 'submit')."</td>
	</tr>
	</table>
	</form>";
...
/* No need, is already donne via JS and dialog box....
if ($action == 'confcache')
{
	$text = "
	<form method='post' action='".e_SELF."?mode=prefs&action=maintenance&cache_clear'>
	<table class='table adminform'>
	<tr>
		<td>".EC_ADLAN_A162." </td>
	</tr>
...
{
	$text = "
	<form method='post' action='".e_SELF."?mode=prefs&action=maintenance'>
...
";
*/
$text.="			<span class='field-help'><em>".EC_ADLAN_A143."</em></span>	".$frm->admin_button('deleteold', EC_ADLAN_A145, 'submit')."
		</td>
	</tr>
";
